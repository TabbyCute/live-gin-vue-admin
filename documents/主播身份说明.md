...
| `total_live_duration` | `TotalLiveDuration` / `totalLiveDuration` | `bigint unsigned`，默认 `0` | 累计直播时长，单位为秒 | APP 本人、APP 公开、后台 |
...
超级管理员 `888` 在系统首次补建该菜单时会获得初始菜单和接口权限。其他管理员角色需要在“角色管理 / API 权限 / 菜单权限”中按职责单独授权。
...
> 当前模块只负责主播身份和资格判断，不负责创建直播间、推流地址、StreamKey、LiveKit Room 或直播 Session。
...
- 不会因为后台审核通过而自动赋值；
- 不代表某一场直播实际选择的分类。以后增加直播场次或直播间模块时，单场直播分类应保存在直播场次/直播间记录中，不能只依赖主播的长期主要分类；
...
- 只有已停用、没有子分类且未被 `live_anchor.category_id` 引用的分类可以删除；
...
- 运营：`isSigned`；
- 统计：`fansCount`、`totalLiveCount`、`totalLiveDuration`、`maxOnlineCount`、`totalViewCount`、`lastLiveAt`、`lastLiveEndAt`。
...
1. 有效的管理后台 `x-token`；
2. 当前角色在 Casbin 中拥有相应 API 路径权限；
3. 对应 API 已同步到 `sys_apis`。

主播接口不会自动授权给所有管理员角色，需要在现有 GVA 权限配置流程中显式授权。
...
2. **确认主播记录**：调用 APP `/anchor/info`，确认 `isAnchor`、`id`、`anchorNo`；
